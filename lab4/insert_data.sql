...
INSERT INTO `quiz_question` (question_id, quiz_id, text, type, answer, order_index)
VALUES (UUID_TO_BIN(@q1), UUID_TO_BIN(@c5), 'How long does the fish spin?', 'multiple_choice', '24 hours', 1),
       (UUID_TO_BIN(@q2), UUID_TO_BIN(@c5), 'The main character of Tihii Den', 'multiple_choice', 'cleaner', 2),
       (UUID_TO_BIN(@q3), UUID_TO_BIN(@c5), 'Top 3 docker commands to broke a container', 'sequence', 'docker pull,docker build,docker run', 3),
       (UUID_TO_BIN(@q4), UUID_TO_BIN(@c6), 'The best ways to name folder', 'multiple_choice', 'mv -r /opt ~/super-folder,super-folder', 1),
       (UUID_TO_BIN(@q5), UUID_TO_BIN(@c6), 'Sort these folder names', 'sequence', 'super-folder,super-folder2,super-final-folder,super-final-folder-ultra',2),
       (UUID_TO_BIN(@q6), UUID_TO_BIN(@c6), 'How long does the fish spin?', 'multiple_choice', '24 hours', 3);
...
VALUES (UUID_TO_BIN(@a1), UUID_TO_BIN(@q1), 'ав'),
       ...
       (UUID_TO_BIN(@a3), UUID_TO_BIN(@q4), 'mv -r /opt ~/super-folder,super-folder'),
       ...
       (UUID_TO_BIN(@a5), UUID_TO_BIN(@q4), 'mv -r /opt ~/super-folder,super-folder'),
